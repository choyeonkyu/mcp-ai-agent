...
# 3. FastAPI lifespan을 사용한 에이전트 준비
@asynccontextmanager
async def lifespan(app: FastAPI):
    """FastAPI 애플리케이션의 생명주기 동안 MCP 연결 및 에이전트 설정을 관리합니다."""
    print("애플리케이션 시작: MCP 서버에 연결하고 에이전트를 설정합니다...")

    async with streamablehttp_client("http://localhost:8000/mcp") as (read, write, _): # _ 인자: 세션 ID는 알 필요 없기 때문
...
async def stream_agent_response(agent_executor, message: str, session_id: str):
    ...
    try:
        config = {"configurable": {"thread_id": session_id}} # script.js에서 생성된 session_id
